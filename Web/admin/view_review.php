<?php 
 ...
 <?php 
  ...
  if (isset($_GET["page"])) {
    ...
    $listsql0 = "select * from review";
    $listsel0 = mysql_query($listsql0,$conn);
    $listnum = mysql_num_rows($listsel0);
   $sql = "select * from review limit $temp,$list_num";
  ...
  <td><a href="del_review.php?&&reviewid=<?php echo $res['reviewid'];?>&&content=<?php echo $content; ?>"onclick="return confirmAct();"><input type="submit" value="删除"></a></td>
   ...
   <?php 
    ...
    echo "<a href='./view_review.php?page=1'>首页</a> | ";
    }
    if ($pre_page<1) {
    echo "上页 | ";
    } 
    else{
    echo "<a href = './view_review.php?page=$pre_page'>上页</a> | ";
     }
   if ($next_page > $p_count) {
    echo "下页 | ";
   }else{
    echo "<a href='./view_review.php?page=$next_page'>下页</a> | ";
  }
  if ($page>$p_count) {
     echo "尾页 </br>";
    }else{
    echo "<a href='./view_review.php?page=$p_count'>尾页</a></br>";
      }     
