<?php 
 ...
 <?php 
  ...
  <?php 
    ...
    $sql2 = "delete from new where newid='$newid'";
    $del = mysql_query($sql,$conn);
    $del2 = mysql_query($sql2,$conn);
    if ($del and $del2 and empty($content)) {
     echo "<script> window.location.href='view_new.php';
           </script>;";
  ...
  <?php
    ...
    $listsql0 = "select * from new where concat(newid,new_content,userid,new_userid,new_userid_number,textid,new_date) like  '%".$content."%' ";
    $listsel0 = mysql_query($listsql0,$conn); 
    $listnum = mysql_num_rows($listsel0);
 if (isset($_GET['content'])|| !empty($_GET['content']))  {  
   $content = $_GET['content'];
   $sql = "select * from new where concat(newid,new_content,userid,new_userid,new_userid_number,textid,new_date) like  '%".$content."%' limit $temp,$list_num ";
  ...
  <?php while ($res = mysql_fetch_array($sel)) {
   ...
   <?php 
    ...
    echo "<a href='./view_search_new.php?page=1&&content=$content'>首页</a> | ";
    }
    if ($pre_page<1) {
    echo "上页 | ";
    } 
    else{
    echo "<a href = './view_search_new.php?page=$pre_page&&content=$content'>上页</a> | ";
     }
   if ($next_page > $p_count) {
    echo "下页 | ";
   }else{
    echo "<a href='./view_search_new.php?page=$next_page&&content=$content'>下页</a> | ";
  }
  if ($page>$p_count) {
     echo "尾页 </br>";
    }else{
    echo "<a href='./view_search_new.php?page=$p_count&&content=$content'>尾页</a></br>";
      }     
